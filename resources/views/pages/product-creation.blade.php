    ...
    <form class="admin-section" id="productForm" action="{{ url('/administracion/productos/creacion-edicion') }}" method="POST">
        ...
        <div class="row col-12">
            ...
            <div class="col-12">
                ...
                <textarea class="form-input" name="description" id="description" cols="20" rows="10" placeholder="Descripción"></textarea>
            </div>
            ...
            <div class="col-4 mt-2 mb-4">
                ...
                <select class="form-input" name="color_id" id="color_id">
                    @if ($categories->count() > 1)
                        <option class="form-input" value="">Seleccioná un tipo de color</option>
                    @endif
                    @foreach ($colors as $color)
                        <option class="form-input" value="{{ $color->id }}">{{ $color->color }}</option>
                    @endforeach
                </select>
            </div>

            <div class="col-12">
                <button class="btn button-filter w-100" style="margin-top: 15px;" type="submit">Listar Producto</button>
            </div>
        </div>
    </form>
    <script type="module">
